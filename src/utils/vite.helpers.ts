...
export const createReactApp = async (
  ...
  try {
    ...
    await createWagmiConfigFile(projectPathOrName);
    await updateMainFile(projectPathOrName);
    await updateAppFile(projectPathOrName);
    await createClientProvider(projectPathOrName);
    await createTalwindConfig(projectPathOrName);
    await updateIndexCss(projectPathOrName);
